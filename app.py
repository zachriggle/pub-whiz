...
for k,v in beerData.items():
    result = v['result']
    if not result or not re.match(urlPattern, result['Url']):
        del beerData[k]
...
for beer in beerNames:
    ...
    for result in results: #sorted(results, key=sortKey,reverse=True):
...
def getScore((a,b)):
    try:
        return int(b['score']) # 0 # return data['score']
    except:
        return -1
